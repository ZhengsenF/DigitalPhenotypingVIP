...
python recognition.py D:/maskRCNN/weights/leaf_little.h5 D:/maskRCNN/recognition_target_single/ D:/maskRCNN/recognition/
"""
...
def detect_and_color_splash(model, image_path=None, video_path=None, save_path=None):
    ...
    if image_path:
        ...
        # find RGB average of the mask
        for eachBB, maskIndex in zip(r['rois'], range(0, len(r['rois']))):
            total_red = 0
            total_green = 0
            total_blue = 0
            total_pixel = 0  # total pixel number
            for topBot in range(eachBB[0], eachBB[2]):
                for leftRight in range(eachBB[1], eachBB[3]):
                    if r['masks'][topBot][leftRight][maskIndex]:
                        total_red += orig_image[topBot][leftRight][0]
                        total_green += orig_image[topBot][leftRight][1]
                        total_blue += orig_image[topBot][leftRight][2]
                        total_pixel += 1
            average_red = total_red / total_pixel
            average_green = total_green / total_pixel
            average_blue = total_blue / total_pixel
            average_RGB[0].append(average_red)
            average_RGB[1].append(average_green)
            average_RGB[2].append(average_blue)
    ...
    return num_spikes, pixel_count, spike_height, spike_width, center_mask, average_RGB
...
if __name__ == '__main__':
    ...
    for eachTarget in targets:
        ...
        average_RGB_main[0] += output[5][0]
        average_RGB_main[1] += output[5][1]
        average_RGB_main[2] += output[5][2]

    output_dict = {'file name': file_name_main, 'spike index': num_spikes_main, 'pixel count': pixel_count_main,
                   'spike height': spike_height_main, 'spike width': spike_width_main,
                   'center of mask': center_mask_main, 'average red': average_RGB_main[0],
                   'average green': average_RGB_main[1], 'average blue': average_RGB_main[2]}
